...
func GetPlayPage(ctx context.Context) (page *pages_service.PlayPage) {
	if page, _ = ctx.Value(pages_service.CtxKeyPlayPage).(*pages_service.PlayPage); page == nil {
...
func GetSession(ctx context.Context) (session *internal.Session) {
	if session, _ = ctx.Value(internal.CtxKeySession).(*internal.Session); session == nil {
